          ...
          for (const line of lines) {
            if (line.startsWith('项目：')) {
              ...
              if (projectPath && !projectMap.has(projectPath)) {
                try {
                  const remoteResult = await shell.exec('git', ['config', '--get', 'remote.origin.url'], { cwd: projectPath });
                  projectMap.set(projectPath, remoteResult.success && remoteResult.stdout ? remoteResult.stdout.trim() : 'none');
                } catch {
